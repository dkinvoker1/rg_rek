// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'event_game_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

EventGameModel _$EventGameModelFromJson(Map<String, dynamic> json) {
  return _EventGameModel.fromJson(json);
}

/// @nodoc
mixin _$EventGameModel {
  double get argument => throw _privateConstructorUsedError;
  int get combinationType => throw _privateConstructorUsedError;
  int get eventLayout => throw _privateConstructorUsedError;
  int get gameCode => throw _privateConstructorUsedError;
  double get gameId => throw _privateConstructorUsedError;
  int get gameLayout => throw _privateConstructorUsedError;
  String get gameName => throw _privateConstructorUsedError;
  int get gameType => throw _privateConstructorUsedError;
  List<int> get marketTypes =>
      throw _privateConstructorUsedError; // required int periodId,
  List<GameOutcomeModel> get outcomes => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $EventGameModelCopyWith<EventGameModel> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $EventGameModelCopyWith<$Res> {
  factory $EventGameModelCopyWith(
          EventGameModel value, $Res Function(EventGameModel) then) =
      _$EventGameModelCopyWithImpl<$Res, EventGameModel>;
  @useResult
  $Res call(
      {double argument,
      int combinationType,
      int eventLayout,
      int gameCode,
      double gameId,
      int gameLayout,
      String gameName,
      int gameType,
      List<int> marketTypes,
      List<GameOutcomeModel> outcomes});
}

/// @nodoc
class _$EventGameModelCopyWithImpl<$Res, $Val extends EventGameModel>
    implements $EventGameModelCopyWith<$Res> {
  _$EventGameModelCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? argument = null,
    Object? combinationType = null,
    Object? eventLayout = null,
    Object? gameCode = null,
    Object? gameId = null,
    Object? gameLayout = null,
    Object? gameName = null,
    Object? gameType = null,
    Object? marketTypes = null,
    Object? outcomes = null,
  }) {
    return _then(_value.copyWith(
      argument: null == argument
          ? _value.argument
          : argument // ignore: cast_nullable_to_non_nullable
              as double,
      combinationType: null == combinationType
          ? _value.combinationType
          : combinationType // ignore: cast_nullable_to_non_nullable
              as int,
      eventLayout: null == eventLayout
          ? _value.eventLayout
          : eventLayout // ignore: cast_nullable_to_non_nullable
              as int,
      gameCode: null == gameCode
          ? _value.gameCode
          : gameCode // ignore: cast_nullable_to_non_nullable
              as int,
      gameId: null == gameId
          ? _value.gameId
          : gameId // ignore: cast_nullable_to_non_nullable
              as double,
      gameLayout: null == gameLayout
          ? _value.gameLayout
          : gameLayout // ignore: cast_nullable_to_non_nullable
              as int,
      gameName: null == gameName
          ? _value.gameName
          : gameName // ignore: cast_nullable_to_non_nullable
              as String,
      gameType: null == gameType
          ? _value.gameType
          : gameType // ignore: cast_nullable_to_non_nullable
              as int,
      marketTypes: null == marketTypes
          ? _value.marketTypes
          : marketTypes // ignore: cast_nullable_to_non_nullable
              as List<int>,
      outcomes: null == outcomes
          ? _value.outcomes
          : outcomes // ignore: cast_nullable_to_non_nullable
              as List<GameOutcomeModel>,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$_EventGameModelCopyWith<$Res>
    implements $EventGameModelCopyWith<$Res> {
  factory _$$_EventGameModelCopyWith(
          _$_EventGameModel value, $Res Function(_$_EventGameModel) then) =
      __$$_EventGameModelCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {double argument,
      int combinationType,
      int eventLayout,
      int gameCode,
      double gameId,
      int gameLayout,
      String gameName,
      int gameType,
      List<int> marketTypes,
      List<GameOutcomeModel> outcomes});
}

/// @nodoc
class __$$_EventGameModelCopyWithImpl<$Res>
    extends _$EventGameModelCopyWithImpl<$Res, _$_EventGameModel>
    implements _$$_EventGameModelCopyWith<$Res> {
  __$$_EventGameModelCopyWithImpl(
      _$_EventGameModel _value, $Res Function(_$_EventGameModel) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? argument = null,
    Object? combinationType = null,
    Object? eventLayout = null,
    Object? gameCode = null,
    Object? gameId = null,
    Object? gameLayout = null,
    Object? gameName = null,
    Object? gameType = null,
    Object? marketTypes = null,
    Object? outcomes = null,
  }) {
    return _then(_$_EventGameModel(
      argument: null == argument
          ? _value.argument
          : argument // ignore: cast_nullable_to_non_nullable
              as double,
      combinationType: null == combinationType
          ? _value.combinationType
          : combinationType // ignore: cast_nullable_to_non_nullable
              as int,
      eventLayout: null == eventLayout
          ? _value.eventLayout
          : eventLayout // ignore: cast_nullable_to_non_nullable
              as int,
      gameCode: null == gameCode
          ? _value.gameCode
          : gameCode // ignore: cast_nullable_to_non_nullable
              as int,
      gameId: null == gameId
          ? _value.gameId
          : gameId // ignore: cast_nullable_to_non_nullable
              as double,
      gameLayout: null == gameLayout
          ? _value.gameLayout
          : gameLayout // ignore: cast_nullable_to_non_nullable
              as int,
      gameName: null == gameName
          ? _value.gameName
          : gameName // ignore: cast_nullable_to_non_nullable
              as String,
      gameType: null == gameType
          ? _value.gameType
          : gameType // ignore: cast_nullable_to_non_nullable
              as int,
      marketTypes: null == marketTypes
          ? _value._marketTypes
          : marketTypes // ignore: cast_nullable_to_non_nullable
              as List<int>,
      outcomes: null == outcomes
          ? _value._outcomes
          : outcomes // ignore: cast_nullable_to_non_nullable
              as List<GameOutcomeModel>,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_EventGameModel extends _EventGameModel {
  _$_EventGameModel(
      {required this.argument,
      required this.combinationType,
      required this.eventLayout,
      required this.gameCode,
      required this.gameId,
      required this.gameLayout,
      required this.gameName,
      required this.gameType,
      required final List<int> marketTypes,
      required final List<GameOutcomeModel> outcomes})
      : _marketTypes = marketTypes,
        _outcomes = outcomes,
        super._();

  factory _$_EventGameModel.fromJson(Map<String, dynamic> json) =>
      _$$_EventGameModelFromJson(json);

  @override
  final double argument;
  @override
  final int combinationType;
  @override
  final int eventLayout;
  @override
  final int gameCode;
  @override
  final double gameId;
  @override
  final int gameLayout;
  @override
  final String gameName;
  @override
  final int gameType;
  final List<int> _marketTypes;
  @override
  List<int> get marketTypes {
    if (_marketTypes is EqualUnmodifiableListView) return _marketTypes;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_marketTypes);
  }

// required int periodId,
  final List<GameOutcomeModel> _outcomes;
// required int periodId,
  @override
  List<GameOutcomeModel> get outcomes {
    if (_outcomes is EqualUnmodifiableListView) return _outcomes;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_outcomes);
  }

  @override
  String toString() {
    return 'EventGameModel(argument: $argument, combinationType: $combinationType, eventLayout: $eventLayout, gameCode: $gameCode, gameId: $gameId, gameLayout: $gameLayout, gameName: $gameName, gameType: $gameType, marketTypes: $marketTypes, outcomes: $outcomes)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_EventGameModel &&
            (identical(other.argument, argument) ||
                other.argument == argument) &&
            (identical(other.combinationType, combinationType) ||
                other.combinationType == combinationType) &&
            (identical(other.eventLayout, eventLayout) ||
                other.eventLayout == eventLayout) &&
            (identical(other.gameCode, gameCode) ||
                other.gameCode == gameCode) &&
            (identical(other.gameId, gameId) || other.gameId == gameId) &&
            (identical(other.gameLayout, gameLayout) ||
                other.gameLayout == gameLayout) &&
            (identical(other.gameName, gameName) ||
                other.gameName == gameName) &&
            (identical(other.gameType, gameType) ||
                other.gameType == gameType) &&
            const DeepCollectionEquality()
                .equals(other._marketTypes, _marketTypes) &&
            const DeepCollectionEquality().equals(other._outcomes, _outcomes));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(
      runtimeType,
      argument,
      combinationType,
      eventLayout,
      gameCode,
      gameId,
      gameLayout,
      gameName,
      gameType,
      const DeepCollectionEquality().hash(_marketTypes),
      const DeepCollectionEquality().hash(_outcomes));

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_EventGameModelCopyWith<_$_EventGameModel> get copyWith =>
      __$$_EventGameModelCopyWithImpl<_$_EventGameModel>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_EventGameModelToJson(
      this,
    );
  }
}

abstract class _EventGameModel extends EventGameModel {
  factory _EventGameModel(
      {required final double argument,
      required final int combinationType,
      required final int eventLayout,
      required final int gameCode,
      required final double gameId,
      required final int gameLayout,
      required final String gameName,
      required final int gameType,
      required final List<int> marketTypes,
      required final List<GameOutcomeModel> outcomes}) = _$_EventGameModel;
  _EventGameModel._() : super._();

  factory _EventGameModel.fromJson(Map<String, dynamic> json) =
      _$_EventGameModel.fromJson;

  @override
  double get argument;
  @override
  int get combinationType;
  @override
  int get eventLayout;
  @override
  int get gameCode;
  @override
  double get gameId;
  @override
  int get gameLayout;
  @override
  String get gameName;
  @override
  int get gameType;
  @override
  List<int> get marketTypes;
  @override // required int periodId,
  List<GameOutcomeModel> get outcomes;
  @override
  @JsonKey(ignore: true)
  _$$_EventGameModelCopyWith<_$_EventGameModel> get copyWith =>
      throw _privateConstructorUsedError;
}
